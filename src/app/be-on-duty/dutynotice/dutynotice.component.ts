...
@Component({
  ...
  templateUrl: './dutynotice.component.html',
  styleUrls: ['./dutynotice.component.scss']
})
export class DutynoticeComponent implements OnInit {
  ...
  queryTableData() {
    ...
    // this.service.queryDutynotice(param).toPromise().then((res: any) => {
    //   if (res.code === '0') {
  ...
  detail(notice: any, template: TemplateRef<any>) {
    this.files = [];
    this.curNoticeName = notice.name;
    this.service.readNotice({ item: { id: notice.id } }).toPromise().then((res: any) => {
      if (res.code === '0') {
